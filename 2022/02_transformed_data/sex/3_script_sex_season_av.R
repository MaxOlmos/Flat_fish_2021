...
# -------------------------------------------------------------------------
CPUE_new <- NULL
  CPUE_catchability_s%>% filter(!is.na(prop))
  CPUE_catchability_s%>% filter(is.na(prop))
...
png(paste('Season_sex_CPUE1_new.png',sep=''), res=600,width = 32,
    height = 32,units="cm")
p <- ggplot() +
  geom_polygon(data=map_data("world") , aes(x=long, y = lat, group = group), fill="grey")+
  ...
  geom_point(data=CPUE_catchability_s %>% filter(is.na(prop),year %in% c(2001:2010)),aes(x=long,y=lat),col="red",shape=4,size=3)+
  #lims(x = c(0.5, 3.5), y = c(0.5, 3.5)) +
  # scale_size("size", range = c(5, 10), breaks = 2^(1:3)) +
...
png(paste('Season_sex_CPUE2_new.png',sep=''), res=600,width = 32,
    height = 32,units="cm")
p <- ggplot() +
  geom_polygon(data=map_data("world") , aes(x=long, y = lat, group = group), fill="grey")+
  ...
  geom_point(data=CPUE_catchability_s %>% filter(is.na(prop),year %in% c(2011:2019)),aes(x=long,y=lat),col="red",shape=4,size=3)+
  #lims(x = c(0.5, 3.5), y = c(0.5, 3.5)) +
  # scale_size("size", range = c(5, 10), breaks = 2^(1:3)) +
  scale_fill_manual(values = c( "#009900AA","#FF9933AA")) +  theme(legend.box = "horizontal") +
  coord_fixed(xlim = xlims,ylim=ylims) +
  theme_bw()
q <- p + facet_grid(year~ Season)
print(q)
