...
P_cold_gut_index <- D_cold_gut_temp %>% mutate(value_P= value*index) %>%
  group_by(Years,Seasons) %>% 
  summarise(Pvalue= sum(value_P)) %>% 
  mutate(Env.Cond = "Cold")
...
O_uv <- O_uvt %>% group_by(Seasons,Env.Cond) %>% summarise(Overlap_m=mean(Overlap))
...
p <- ggplot() + 
  geom_line(O_uvt_2, mapping=aes(x=Years, y=Overlap_m,color=Env.Cond),size=5,alpha=0.2)+
  ylab("Overlap")+
  geom_line(O_uvt_2, mapping=aes(x=Years, y=Overlap),size=1)+
...
# add survey abundance index ----------------------------------------------
# -------------------------------------------------------------------------
load("04_outputs/EBS_grid/EnvCxSeason/survey/fit.RData")
fit_s <- fit
fit_s$data_frame
Index_ab <- as.data.frame(fit_s$Report$Index_ctl[,20:37,])
colnames(Index_ab) <- c("survey_IA")

Index_ab <- Index_ab %>%
  mutate(Years=c(2001:2018), Seasons="Int.") %>%
  mutate(IA_index = (survey_IA-mean(survey_IA))/sd(survey_IA)) %>% dplyr::select(Years,Seasons, IA_index)
Index_ab <- Index_ab %>% mutate(Type="IA") %>% mutate(Index=IA_index) %>% dplyr::select("Years","Index","Type")
...
Index_comp <- rbind(Index_ab,O_index_int)
...
ggsave(paste0(work_dir,'Overlap_ab.png'), p)
getwd()
