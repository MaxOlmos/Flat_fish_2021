...
png(paste(outputs_file,"/Interractions_",'.png',sep=''), height = 6, width = 10, units = 'in', res=600)
par(mfrow=c(1,2))
with(CPUE_sex %>% filter(Sex=='F',CPUE.mean>0),interaction.plot(Season, CP, log(CPUE.mean)))
with(CPUE_sex %>% filter(Sex=='F',CPUE.mean>0),interaction.plot(CP, Season, log(CPUE.mean)))
dev.off()

png(paste(outputs_file,"/Interractions_M",'.png',sep=''), height = 6, width = 10, units = 'in', res=600)
par(mfrow=c(1,2))
with(CPUE_sex %>% filter(Sex=='M',CPUE.mean>0),interaction.plot(Season, CP, log(CPUE.mean)))
with(CPUE_sex %>% filter(Sex=='M',CPUE.mean>0),interaction.plot(CP, Season, log(CPUE.mean)))
dev.off()

# -- Covariates -----------------------------------------------------------
settings$Options["report_additional_variables"]=TRUE

CPUE <- CPUE_sex %>% filter(Sex==sex,CPUE.mean > 0, year > 2000)

p <- ggplot() +geom_point(CPUE_sex, mapping=aes(x=lat, y=long,color=CPUE.mean))

# Build model matrix
Q1_formula = ~   CP:Season
Model_matrix1 = model.matrix( update.formula(Q1_formula, ~.), data=CPUE)
...
CPUE_catchability <- as.data.frame(CPUE)
sort(unique(CPUE_catchability$year))
