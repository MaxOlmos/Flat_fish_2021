...
mdl <- FishStatsUtils:: make_map_info(Region ="user",
...
gmap_effect <- ggplot(ak_map_effect, aes(x = long, y = lat, group = group)) +
  ...
  theme(axis.title.x=element_blank(),
        ...
        panel.grid.minor = element_blank(), panel.ontop = TRUE,  panel.background= element_rect(color = NA, fill = NA)
  ) +
...
g <- 
  ...
  geom_point(data=phi, aes(Lon, Lat, color=(phi), group=NULL),
   ...
   geom_contour( data=bathy.dat,aes(x = lon, y = lat,z = depth,group=NULL),breaks  = c(-200,-100,-50),col="green4",size=1)
