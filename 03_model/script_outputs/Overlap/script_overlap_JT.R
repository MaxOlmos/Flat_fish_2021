...
colnames(O_uv) <- c("Season", "Overlap", "EnvCond")

ggplot() + geom_point(O_uv, mapping=aes(x=Season, y=Overlap, color=EnvCond))
...
D_warm_gu <- array(NA,dim=c(n_knot,length(warm_season)))
  ...
  for ( u in 1 :length(cold_season)){
    for ( g in 1:n_knot){
      D_cold_gu[g,u] <- exp(fit_f$Report$Omega1_gc[g,1])*a_k[g,1] * exp(fit_f$Report$Phi1_sk[g,cold_season[u]]) 
    }
  }
  ...
  for ( u in 1 :length(warm_season)){
    for ( g in 1:n_knot){
      D_warm_gu[g,u] <- exp(fit_f$Report$Omega1_gc[g,1])*a_k[g,1]* exp(fit_f$Report$Phi1_sk[g,warm_season[u]])  
    }
  }
...
p<-ggplot(O_uv_omega, mapping=aes(x=Season, y=Overlap, color=EnvCond)) +
  scale_color_manual(values=c( "#1E90FF","#FF0000"))+   geom_point()+geom_line()

png(paste('Overlap','.png',sep=''), height =14 , width = 20, units = 'cm', res=500)#, res=500)
p
dev.off()
